...
const SENSITIVE_ASSIGNMENT = /["']?\b(?:authorization|cookie|set-cookie|password|passwd|secret|token|access[_-]?token|refresh[_-]?token|client[_-]?(?:id|secret)|database[_-]?url|code|state|id[_-]?token|email|preferred[_-]?username|tenant[_-]?id|tid|acct|sub|iss|aud|name|session(?:[_-]?token)?|csrf(?:[_-]?token)?|nonce|verifier|redirect[_-]?uri|authorization[_-]?code|oauth[_-]?(?:code|state))\b["']?\s*[:=]\s*(?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^\s,;)}\]]+)/gi;
