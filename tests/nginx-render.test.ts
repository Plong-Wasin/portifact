...
function run(appHost: string, maxBytes: string): number {
  const r = spawnSync("sh", ["-c", `APP_HOST=${JSON.stringify(appHost)} MAX_ARTIFACT_CONTENT_BYTES=${JSON.stringify(maxBytes)} sh ${SCRIPT}`], { encoding: "utf8" });
  return r.status ?? -1;
}
...
describe.skipIf(skip)("nginx env render validation", () => {
  ...
  test("rejects zero MAX_ARTIFACT_CONTENT_BYTES", () => {
    expect(run("example.com", "0")).not.toBe(0);
  });

  test("rejects non-integer MAX_ARTIFACT_CONTENT_BYTES", () => {
    expect(run("example.com", "1m")).not.toBe(0);
  });

  test("rejects empty MAX_ARTIFACT_CONTENT_BYTES", () => {
    expect(run("example.com", "")).not.toBe(0);
  });

  test("rejects missing APP_HOST", () => {
    const r = spawnSync("sh", ["-c", `MAX_ARTIFACT_CONTENT_BYTES=1048576 sh ${SCRIPT}`], { encoding: "utf8" });
